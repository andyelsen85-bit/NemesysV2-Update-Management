...
import type {
  ApiKeyRotation, AuditEntry, Client, ExeCheck, IniCheck, IniRule, ServerSettings,
  SoftwarePolicy, SoftwarePolicyInput, SyncConfig
} from '@workspace/api-client-react';
  ...
  return <div className="mx-auto max-w-[1380px]">
    ...
    {dashboard.isError ? <ErrorState onRetry={retry} /> : anyLoading && !summary ? <LoadingRows count={5} /> : <>
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
       ...
       <section data-testid="panel-global-update-mode" className={cx('mt-6 overflow-hidden rounded-xl border transition-colors', updateMode ? 'border-[#b4d5c8] bg-[#e5f3ec]' : 'border-[#ead9cc] bg-[#fbf0e7]')}>
         <div className="flex flex-col gap-5 px-5 py-5 md:flex-row md:items-center md:justify-between md:px-6">
           <div className="flex items-start gap-4"><div className={cx('mt-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-xl', updateMode ? 'bg-[#c7e5d8] text-[#297660]' : 'bg-[#f6dccc] text-[#b5613c]')}>{updateMode ? <ShieldCheck size={22} /> : <AlertTriangle size={22} />}</div><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-extrabold tracking-tight text-[#263d3e]">Global Update Mode</h2><StatusPill value={updateMode ? 'active' : 'standby'} kind={updateMode ? 'success' : 'warning'} /></div><p data-testid="text-update-mode-description" className="mt-1.5 max-w-xl text-xs leading-5 text-[#718481]">{updateMode ? 'Policies use the shortened close-on-start timeout while applications enter their controlled update window.' : 'Enable before maintenance to let policies safely close running applications and apply pending updates.'}</p><div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-[10px] text-[#69817c]"><span className="inline-flex items-center gap-1.5"><Clock3 size={13} />Normal close-on-start: <b className="text-[#3d5c59]">30 sec</b></span><span className="text-[#b0bfba]">→</span><span className="inline-flex items-center gap-1.5"><Activity size={13} />Shortened close-on-start: <b className={updateMode ? 'text-[#24775f]' : 'text-[#3d5c59]'}>8 sec</b></span></div></div></div>
           <button type="button" aria-pressed={updateMode} data-testid="toggle-global-update-mode" onClick={() => setUpdateMode((current) => !current)} className={cx('relative inline-flex h-9 w-[158px] shrink-0 items-center rounded-full p-1 transition-colors', updateMode ? 'bg-[#277b64]' : 'bg-[#aebfba]')}><span className={cx('absolute flex size-7 items-center justify-center rounded-full bg-[#fbfdfb] shadow-[0_2px_5px_rgba(30,65,60,.22)] transition-transform', updateMode ? 'translate-x-[122px]' : 'translate-x-0')}>{updateMode ? <Check size={14} className="text-[#2d8069]" /> : <span className="size-1.5 rounded-full bg-[#aebfba]" />}</span><span className={cx('w-full text-center text-[11px] font-bold', updateMode ? 'pr-5 text-[#effaf5]' : 'pl-5 text-[#f6fbf8]')}>{updateMode ? 'Disable mode' : 'Enable mode'}</span></button>
         </div>
         {updateMode && <div data-testid="status-global-update-mode" className="flex items-center gap-2 border-t border-[#cde2d8] bg-[#d9eee4] px-5 py-2.5 text-[11px] font-semibold text-[#347661] md:px-6"><Activity size={14} className="animate-pulse" />Update Mode is enabled locally for this preview; backend persistence is not yet available.</div>}
       </section>
...
function SettingsPage() {
  ...
  return <div className="mx-auto max-w-[1080px]">
    ...
    {query.isError ? <ErrorState onRetry={() => query.refetch()} /> : query.isLoading && !initialized ? <LoadingRows count={4} /> : <form onSubmit={save} className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]"><div className="space-y-6">
      ...
      <section data-testid="panel-update-timeouts" className="rounded-xl border border-[#dbe3dd] bg-[#fffdf8] p-5 shadow-[0_4px_18px_rgba(39,66,58,.035)]"><div className="mb-5 flex items-start gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#dfeef1] text-[#286b76]"><Clock3 size={18} /></div><div><h2 className="text-sm font-extrabold text-[#284139]">Update Mode and timeouts</h2><p className="mt-1 text-xs text-[#87958e]">These values are stored server-side and apply to the next client sync.</p></div></div><SettingToggle label="Global Update Mode" detail={updateMode ? `Short close-on-start timeout: ${form.updateModeCloseTimeoutSeconds} seconds.` : `Normal close-on-start timeout: ${form.normalCloseTimeoutSeconds} seconds.`} value={updateMode} onChange={setUpdateMode} testId="toggle-settings-update-mode" /><div className="mt-4 grid gap-3 sm:grid-cols-2"><label><span className="field-label">Normal timeout</span><input type="number" min="5" max="3600" required data-testid="input-normal-timeout" value={form.normalCloseTimeoutSeconds} onChange={(e) => set('normalCloseTimeoutSeconds', Number(e.target.value))} className="field-input font-mono" /></label><label><span className="field-label">Short timeout</span><input type="number" min="1" max="300" required data-testid="input-short-timeout" value={form.updateModeCloseTimeoutSeconds} onChange={(e) => set('updateModeCloseTimeoutSeconds', Number(e.target.value))} className="field-input font-mono" /></label></div></section>
