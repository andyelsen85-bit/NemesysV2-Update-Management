...
function PolicyCardUnified({ policy, onEdit }: { policy: SoftwarePolicy; onEdit: () => void }) {
  ...
  return <div data-testid={`card-policy-${policy.id}`} className="rounded-xl border border-[#dbe3dd] bg-[#fffdf8] p-5 shadow-[0_4px_18px_rgba(39,66,58,.035)] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#b7cec1]">
    <div className="flex items-start justify-between gap-3"><div className="flex min-w-0 items-start gap-3"><div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#dff2e9] text-[#247455]"><HardDrive size={18} /></div><div className="min-w-0"><h2 className="truncate text-sm font-extrabold text-[#2d4942]">{policy.name}</h2><div className="mt-1 text-[10px] text-[#8a9992]">Application supervision and compliance checks</div></div></div><button aria-label={`Edit ${policy.name}`} data-testid={`button-edit-policy-${policy.id}`} onClick={onEdit} className="rounded-lg p-2 text-[#79908a] hover:bg-[#e5eee8] hover:text-[#246d53]"><Pencil size={15} /></button></div>
    ...
    <div className="flex items-center justify-between"><div className="flex items-center gap-2"><StatusPill value={policy.enabled ? 'enforcing' : 'paused'} kind={policy.enabled ? 'success' : 'neutral'} /><span className="text-[11px] text-[#87958e]">{policy.graceSeconds}s grace</span></div><span className="font-mono text-[10px] text-[#9aa7a0]">updated {relativeTime(policy.lastUpdated)}</span></div>
