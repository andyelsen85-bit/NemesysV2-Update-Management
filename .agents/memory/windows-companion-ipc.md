...
**How to apply:** Preserve service-owned IPC, first-instance protection, policy gating, and companion process/session validation whenever warning-dialog messages or responses change. Scan cached policy/process state at least every 30 seconds, but run warning/install work asynchronously with one active task per policy and cooldowns to prevent duplicate prompts or installers. Show a warning only for a currently noncompliant policy whose managed process is still running; revalidate compliance, configuration generation, cancellation, and process state before warning and again before closure. Use separate deadlines for connection/authentication and for the user countdown response; start the response deadline only after the warning is flushed. Treat process-enumeration, launch, connection, authentication, response, or confirmed process-closure failure as fail-safe: retry later without running installers.