...
**How to apply:** Preserve service-owned IPC, first-instance protection, policy gating, and companion process/session validation whenever warning-dialog messages or responses change. Show a warning only when a configured managed process is actually running; otherwise update silently and recheck later. Use separate deadlines for connection/authentication and for the user countdown response; start the response deadline only after the warning is flushed. Treat process-enumeration, launch, connection, authentication, response, or confirmed process-closure failure as fail-safe: retry later without running installers.